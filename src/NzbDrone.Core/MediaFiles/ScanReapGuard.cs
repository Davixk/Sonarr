...
namespace NzbDrone.Core.MediaFiles
{
    ...
    public class ScanReapGuard : IScanReapGuard, IHandle<ApplicationStartedEvent>
    {
        ...
        public void Handle(ApplicationStartedEvent message)
        {
            ...
            _logger.Info("fork14 config: dangling-symlink reaper {0} (storage health via target walk-up, ENOENT-gap aware), cleanup empty-enum bail on, SQLITE_BUSY_TIMEOUT={1}ms, probe kill-on-timeout on (all spawn sites), IMPORT_PROBE_THREADS={2} IMPORT_PROBE_TIMEOUT={3}s IMPORT_PROBE_TIMEOUT_STRIKES={4}",
